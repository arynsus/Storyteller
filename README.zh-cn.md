...
2. 可以使用 `窗口` 菜单中的 `电子书加载器` 加载 EPUB、FB2 或整卷 `.txt` 文件。若识别到目录，章节会以勾选列表形式列出，可勾选需要的章节；若没有可用目录，则可通过正则切割模式手动定义分割规则。一键将结果导入待转换列表。
...
6. 根据需要调整 `文字转语音设置` ，然后单击 `转换`。 请注意，Edge的服务器会拒绝异常频繁的请求，因此，如果不着急，请将 `并行任务数` 和 `并行分块数` 保持为 `1`。另一个思路是将这两个数字设置得极高，Edge的服务器将在初始批次后关闭连接，转换进程将会显示为错误，但此时可以再次单击 `转换` 重试，之前的转换进度会被保留——已完成的分块现在会留在磁盘上，重启应用后同样有效。
7. 打开 `历史` 标签页可浏览所有已转换的内容，按书籍分组。每个章节都可就地播放、下载或移除。移除分为两种：`清理缓存` 只删除该章节的工作文件（原文、残留分块与封面副本），保留音频——此后该章节将无法重新转换或编辑；`删除` 则连同音频一并删除。没有产出音频的任务，以及不属于任何任务的缓存文件，会集中在 `未完成与未归属` 分组中，可一键清理。删除的文件会进入系统回收站。
8. 以上内容都存放在同一个 `content_cache` 文件夹中，可从「历史」中直接打开。`设置 → 存储空间` 用于设定一个容量阈值，超过后历史图标会变红并发送一次通知；该阈值统计整个文件夹（包含有声书本身），因此更适合当作「书库变大了」的提醒，而非垃圾文件警告。
