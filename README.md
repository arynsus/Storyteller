...
2. Use `Ebook Loader` in the `Window` menu to load an EPUB, FB2, or full-volume `.txt` file. Chapters detected from a table of contents show up as a checklist to include/exclude; when there's no usable table of contents, a regex-based Regex Split mode lets you split the text manually, same as before. Import the result into the conversion list with one click.
...
6. Adjust the `TTS Config` as needed, then click `Convert`. Please note that Edge's servers will reject excessively frequent requests. Therefore, if you are not in a hurry, keep the `Concurrent jobs` and `Concurrent sections per job` at `1`. Another approach is to set these two numbers very high. Edge's servers will close the connection after the initial batch, and the conversion process will be marked as an error, but you can click `Convert` again to retry. The previous conversion progress will be retained — including across restarts, since finished sections now stay on disk.
7. Open the `History` tab to browse everything you have converted, grouped by book. Each chapter can be played back in place, downloaded, or removed. Two kinds of removal are offered: `Free cache` deletes a chapter's working files (its source text, leftover sections and cover copy) while keeping the audio — after this the chapter can no longer be re-converted or edited — and `Delete` removes the chapter entirely, audio included. Jobs that never produced audio, together with any cache files belonging to no job, are collected in an `Unfinished & untracked` group that can be cleared in one go. Deletions go to your system trash.
8. Storyteller keeps all of this under a single `content_cache` folder, openable from History. `Settings → Storage` sets a size at which the History icon turns red and a one-off notification is posted; it measures that whole folder, audiobooks included, so treat it as a "my library got big" reminder rather than a junk alert.
